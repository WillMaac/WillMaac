<h1 align="center">👨🏿‍💻 Anderson de Jesus</h1>

<p align="center">
  <strong>Desenvolvedor Frontend</strong><br/>
ReactJS | React Native | Nextjs
</p>



##  Sobre mim

Me chamo Anderson de Jesus, tenho 27 anos e sou Desenvolvedor Frontend Júnior.

Minha entrada na tecnologia não foi por acaso; foi uma escolha consciente: migrar da área da saúde para o desenvolvimento de software, buscando um campo em que eu pudesse construir soluções e gerar impacto real por meio da tecnologia.

Hoje, desenvolvo aplicações web com React, Next.js e TypeScript, sempre priorizando clareza na arquitetura, organização do código e performance. Não me preocupo apenas em “fazer funcionar”, mas em compreender o porquê das decisões técnicas e construir soluções sustentáveis.

Tenho ampliado minha atuação no desenvolvimento mobile com React Native, buscando criar experiências multiplataforma com consistência e alta qualidade.


Atualmente desenvolvendo aplicações com:

<p align="left">
   React
</p>

<p align="left">
   React Native
</p>

<p align="left">
   Next.js
</p>

<p align="left">
  Tailwind CSS
</p>
<p align="left">
   TypeScript
</p>



##  Tecnologias

<p align="center">
  <img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,html,css,tailwind,git,mysql" />
</p>



##  Estatísticas

<p align="center">
  <img height="170" src="https://github-readme-stats-sigma-five.vercel.app/api?username=WillMaac&show_icons=true&theme=tokyonight" />
  <img height="170" src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=WillMaac&layout=compact&theme=tokyonight" />
</p>




##  Contatos

<p align="center">
  <a href="https://www.linkedin.com/in/andersonde-jesus/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>&nbsp;&nbsp;
  <a href="mailto:andersonwj9@gmail.com"><img src="https://img.shields.io/badge/E--mail-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
</p>



<p align="center">
  💡 “Código limpo não é aquele que funciona. É aquele que qualquer desenvolvedor entende.”
</p>
